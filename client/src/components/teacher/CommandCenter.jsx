...
// End Session (deletes everything from memory). Hold the Line is single-role solo,
// so students start on their own as soon as they're approved — no pairing needed.
...
export default function CommandCenter() {
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Hold the Line: The Alamo · everyone commands Travis · TEKS 7.3B, 7.3C</p>
  ...
  const cls = roster?.classAccuracy?.travis || { count: 0, average: 0 };
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Hold the Line: The Alamo · session-only data · download the PDF before you end</p>
        </div>
      ...
      <footer className="app-footer">7th Grade Texas History · Hold the Line: The Alamo · TEKS 7.3B, 7.3C</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [122, 62, 46]; // adobe brick

  doc.setFontSize(16);
  doc.text('Hold the Line: The Alamo — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  doc.text('7th Grade Texas History · Hold the Line: The Alamo · TEKS 7.3B, 7.3C', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`hold-the-line_${roster.joinCode}_${date}.pdf`);
}
