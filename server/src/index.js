...
app.get('/healthz', (_req, res) =>
  res.json({ ok: true, service: 'hold-the-line', liveSessions: manager.registry.sessions.size }));
...
server.listen(config.port, () => {
  console.log(`[hold-the-line] listening on :${config.port} — sessions live in memory only`);
});
