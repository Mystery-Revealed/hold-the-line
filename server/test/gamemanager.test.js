...
// and inspects the emit instructions it returns. No sockets involved. Hold the
// Line is single-role solo, so these focus on the solo lifecycle.
import test from 'node:test';
import assert from 'node:assert/strict';
import { GameManager } from '../src/GameManager.js';
import game from '../src/games/holdTheLine.js';
...
test('the default game is Hold the Line', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  assert.equal(manager.registry.get(joinCode).gameId, 'hold-the-line');
});
...
test('solo student starts immediately when approval is off, and completes with 100% accuracy', () => {
  ...
  assert.equal(begin[0].payload.side, 'travis');
  ...
  assert.equal(end[0].payload.yourSide, 'travis');
  assert.equal(end[0].payload.you.ending.key, 'held', 'a perfect run holds the line');
  assert.ok(end[0].payload.you.holdScore >= 210);
  ...
  assert.equal(roster.classAccuracy.travis.count, 1, 'one class group: travis');
  assert.equal(roster.classAccuracy.travis.average, 100);
});
...
test('a wrong-kind move is rejected', () => {
  ...
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  // Phase 1 step 1 is a map action; submit a decision instead.
...
test('a map move places a marker the client can render', () => {
  ...
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  const out = playRight(manager, joinCode, res.studentId); // phase 1 map action, played right
  const resolution = studentEvents(out.emits, res.studentId, 'turn:resolution')[0];
  assert.ok(resolution.payload.placed, 'resolution reports the placement');
  assert.ok(resolution.payload.map.positions[resolution.payload.placed.position].markers.length >= 1);
});
...
test('rejoin returns a full snapshot of the live turn', () => {
  ...
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  playRight(manager, joinCode, res.studentId); // map move done; a decision is pending
  ...
  assert.ok(back.sync.matchBegin.meta.positions, 'meta ships with the snapshot');
